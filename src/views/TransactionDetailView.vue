...
<template>
  <div class="flex flex-col h-full p-6 relative">
    <PepHeader title="Transaction" :onBack="() => router.push('/dashboard')" :absolute="false" />

    <div v-if="isLoading" class="mt-8 flex-1 flex flex-col items-center justify-center space-y-4">
    ...
    <div v-else-if="error" class="mt-20 flex-1 flex flex-col items-center justify-center space-y-4 text-center">
      <p class="text-red-400 font-bold">{{ error }}</p>
    </div>

    <div v-else-if="txModel" class="mt-12 space-y-8 flex-1 overflow-y-auto pr-1 text-center">
      ...
      <div class="space-y-4 text-left">
        <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-4 space-y-4">
          ...
          <div class="flex justify-between items-baseline pt-4 border-t border-slate-700/30">
