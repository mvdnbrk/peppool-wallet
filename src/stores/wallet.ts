...
async function setAutoLockAlarm(delayMinutes: number) {
    if (typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
        ...
        } catch { /* background worker unavailable (e.g. dev mode) */ }
    }
}
...
async function clearAutoLockAlarm() {
    if (typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
        ...
        } catch { /* background worker unavailable */ }
    }
}
...
interface FailureTier { threshold: number; durationMs: number; wipe?: boolean }
