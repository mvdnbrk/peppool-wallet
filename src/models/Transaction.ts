...
export const RawTransactionSchema = v.object({
  ...
  version: v.number(),
  locktime: v.number(),
  vin: v.array(v.object({
    txid: v.string(),
    vout: v.number(),
    prevout: v.nullable(v.object({
      scriptpubkey_address: v.string(),
      value: v.number(),
    })),
  })),
  vout: v.array(v.object({
    scriptpubkey_address: v.string(),
    value: v.number(),
  })),
  size: v.number(),
...
export class Transaction {
  ...
  get txid() { return this.raw.txid; }
  get fee() { return this.raw.fee / RIBBITS_PER_PEP; }
  get isConfirmed() { return this.raw.status.confirmed; }
  get blockHeight() { return this.raw.status.block_height ?? null; }
  ...
  get txidStart() { return truncateId(this.txid).start; }
  get txidEnd() { return truncateId(this.txid).end; }
  get txidShort() { return `${this.txid.slice(0, 8)}...${this.txid.slice(-8)}`; }
}
